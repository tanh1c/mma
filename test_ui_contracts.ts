...
for (const token of ['personalityTraits.slice(0, 2)', '<details', '$.personality.title', '$.personality.traits.professional']) assert.ok(fighterDetail.includes(token), `Personality UI missing ${token}`);
