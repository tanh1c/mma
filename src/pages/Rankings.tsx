...
export default function Rankings() {
  ...
  const getChampionStatus = () => {
    ...
    if (!champion || !titleInfo) return { label: t($ => $.rankings.active), tone: 'success' as StatusTone, tooltip: t($ => $.rankings.activeTooltip) };
    if (titleInfo.status === 'inactive_champion') return { label: t($ => $.rankings.inactiveChampion), tone: 'danger' as StatusTone, tooltip: t($ => $.rankings.inactiveChampionTooltip) };
    if (titleInfo.status === 'unification_needed') return { label: t($ => $.rankings.unificationNeeded), tone: 'warning' as StatusTone, tooltip: t($ => $.rankings.unificationNeededTooltip) };
    if (champion.lastFightDate) {
      ...
      if (days > 120 && days < 180) return { label: t($ => $.rankings.pendingDefense), tone: 'warning' as StatusTone, tooltip: t($ => $.rankings.pendingDefenseTooltip) };
    }
    return { label: t($ => $.rankings.active), tone: 'success' as StatusTone, tooltip: t($ => $.rankings.activeTooltip) };
  };
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-12">
      <PageHeader eyebrow={t($ => $.rankings.eyebrow)} title={t($ => $.rankings.title)} actions={<Select value={selectedWC} onChange={setSelectedWC} options={wcOptions} className="w-full sm:w-48" />} />
      ...
      {belt && <Panel className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 flex-col items-center gap-3 text-center sm:flex-row sm:text-left">
          <ChampionshipBelt weightClass={selectedWC as WeightClass} type="undisputed" size="hero" alt={t($ => $.rankings.beltAlt, { name: belt.name })} />
          <div className="min-w-0"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-neutral-500">{t($ => $.rankings.currentDivisionBelt)}</p><h2 className="mt-2 text-2xl font-normal tracking-[-0.03em] text-white">{belt.name}</h2><p className="mt-2 text-sm text-neutral-400">{t($ => $.rankings.prestige)} <span className="font-mono text-white">{Math.floor(belt.prestige)} / 100</span></p></div>
        </div>
        <div className="flex w-full flex-col gap-3 lg:w-auto lg:min-w-80">
          {undisputedChampion ? <ChampionCard label={t($ => $.rankings.undisputed)} fighter={undisputedChampion} defenses={titleInfo?.undisputedDefenses ?? 0} status={{ ...championStatus, tooltip: statusTooltip }} defensesLabel={t($ => $.rankings.defenses)} lastFightLabel={t($ => $.rankings.lastFight)} noneLabel={t($ => $.rankings.none)} language={language} onOpen={() => setView('fighter-detail', { fighterId: undisputedChampion.id })} /> : <div className="py-2"><StatusBadge tone="danger">{t($ => $.rankings.vacantTitle)}</StatusBadge></div>}
          {interimChampion && <ChampionCard label={t($ => $.rankings.interimChampion)} fighter={interimChampion} defenses={titleInfo?.interimDefenses ?? 0} defensesLabel={t($ => $.rankings.defenses)} lastFightLabel={t($ => $.rankings.lastFight)} noneLabel={t($ => $.rankings.none)} language={language} weightClass={selectedWC as WeightClass} type="interim" onOpen={() => setView('fighter-detail', { fighterId: interimChampion.id })} />}
        </div>
      </Panel>}

      <DataSurface>
        {currentRankings.length === 0 ? <div className="p-8 text-center text-neutral-500">{t($ => $.rankings.empty)}</div> : <div className="overflow-x-auto custom-scrollbar"><table className="min-w-[560px] w-full text-left text-sm text-neutral-400">
          <thead className="border-b border-[#2a2c31] bg-black/10 font-mono text-[10px] uppercase tracking-[0.14em] text-neutral-500"><tr><th className="w-16 p-4 text-center font-normal">{t($ => $.rankings.columns.rank)}</th><th className="w-16 p-4 text-center font-normal">{t($ => $.rankings.columns.move)}</th><th className="p-4 font-normal">{t($ => $.rankings.columns.fighter)}</th><th className="p-4 font-normal">{t($ => $.rankings.columns.record)}</th><th className="p-4 font-normal">{t($ => $.rankings.columns.status)}</th></tr></thead>
